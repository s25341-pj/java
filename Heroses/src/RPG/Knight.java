...
public class Knight extends Heroses implements Statistic, Lvl{

    //prywatna zmienna poziomu
    private int level;
    int i;

    //utworzenie obiektu rycerza
    Knight(double strenght, double defence, double intelligence, double dextrity, double agility, double speed, int level){
        super(strenght, defence, intelligence, dextrity, agility, speed);
        this.level = level;
    }
    ...
    public void lvlUp() {
        ...
        for (i = 10; i > 0; i--){
            ...
            if (choice < 7 && choice > 0){
                switch (choice) {
                    case 1:
                        super.setStrenght(super.getStrenght() + 1.2);
                        break;
                    case 2:
                        super.setDefence(super.getDefence() + 1.1);
                        break;
                    case 3:
                        super.setIntelligence(super.getIntelligence() + 1.0);
                        break;
                    case 4:
                        super.setDextrity(super.getDextrity() + 1.05);
                        break;
                    case 5:
                        super.setAgility(super.getAgility() + 1.02);
                        break;
                    case 6:
                        super.setSpeed(super.getSpeed() + 1.05);
                        break;
                }
            }else{
